---
title: "GSEA in E12.5 placentas from OgtY841A-homo vs OgtY841A-het females and OgtY841A-hemi vs wt males"
date: '2022-06-27'
author: 
...
This Rmd has to be run after the DE_analysis.Rmd. The reason why it is separated is that GSEA requires lots of memory to run.
...
all_gsea_F <- perform_all_gsea(res_list$res_F)
all_gsea_M <- perform_all_gsea(res_list$res_M)

```

```{r table of GSEA, results='asis'}

knitr::kable(as.data.frame(all_gsea_F[[1]]), caption = "F homo vs F het - Biological Process GO")
knitr::kable(as.data.frame(all_gsea_F[[2]]), caption = "F homo vs F het - Molecular Function GO")
knitr::kable(as.data.frame(all_gsea_F[[3]]), caption = "F homo vs F het - Cellular Component GO")
knitr::kable(as.data.frame(all_gsea_M[[1]]), caption = "M hemi vs M wt - Biological Process GO")
knitr::kable(as.data.frame(all_gsea_M[[2]]), caption = "M hemi vs M wt - Molecular Function GO")
knitr::kable(as.data.frame(all_gsea_M[[3]]), caption = "M hemi vs M wt - Cellular Component GO")

```

```{r GSEA plot (FIGURE), include = TRUE}

# combining gsea results
gsea_M_ALL <- as.data.table(rbind(fortify(all_gsea_M[[1]], showCategory = nrow(all_gsea_M[[1]]@result)), fortify(all_gsea_M[[2]], showCategory = nrow(all_gsea_M[[2]]@result)), fortify(all_gsea_M[[3]], showCategory = nrow(all_gsea_M[[3]]@result))))
gsea_M_ALL$GO <- rep(c("BP", "MF", "CC"), c(nrow(all_gsea_M[[1]]@result), nrow(all_gsea_M[[2]]@result), nrow(all_gsea_M[[3]]@result)))
# computing absolute NES
gsea_M_ALL$abs_NES <- abs(gsea_M_ALL$NES)
# ordering based on abs NES
setorder(gsea_M_ALL, -abs_NES, GO)
# taking first GOs based on NES
best_NES_terms <- gsea_M_ALL[, .SD[1:10], by = "GO", .SDcols = "ID"]$ID
red_gsea_M_ALL <- gsea_M_ALL[ID %in% best_NES_terms]

library(DOSE)
library(tidytext)

ggplot(red_gsea_M_ALL, aes(x = GeneRatio, y = reorder_within(Description, GeneRatio, .sign), color = p.adjust)) +
      ...
      ylab(NULL) +
      theme_dose(10) +
      facet_grid(GO ~ .sign, scales = "free_y") +
